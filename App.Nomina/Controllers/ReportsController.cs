...
public class ReportsController : Controller
{
    ...
    public async Task<IActionResult> CambiosSalarialesPdf(string desde, string hasta, string? q, int? empNo, string? deptNo)
    {
        ...
        var rows = data.Select(x => x.GetType().GetProperties().ToDictionary(p => p.Name, p => p.GetValue(x))).ToList();
        var pdf = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Content().Table(table =>
                {
                    ...
                    foreach (var row in rows)
                    {
                        ...
                        table.Cell().Text(row["Departamento"]?.ToString() ?? "");
                        table.Cell().Text(row["Usuario"]?.ToString() ?? "");
                        table.Cell().Text(((DateTime)row["FechaActualizacion"]).ToString("g"));
                        table.Cell().Text(row["DetalleCambio"]?.ToString() ?? "");
                        table.Cell().Text(((decimal)row["Salario"]).ToString("C"));
                    }
                });
            });
        }).GeneratePdf();
    ...
    public async Task<IActionResult> CambiosSalarialesExcel(string? desde, string? hasta, string? q, int? empNo, string? deptNo)
    {
        DateTime? desdeDt = string.IsNullOrWhiteSpace(desde) ? (DateTime?)null : DateTime.Parse(desde);
        DateTime? hastaDt = string.IsNullOrWhiteSpace(hasta) ? (DateTime?)null : DateTime.Parse(hasta);
        var data = await GetCambiosSalarialesData(desdeDt, hastaDt, q, empNo, deptNo);
        ...
        using var wb = new XLWorkbook();
        var ws = wb.AddWorksheet("Cambios Salariales");
        ws.Cell(1, 1).Value = "ID";
        ws.Cell(1, 2).Value = "Usuario";
        ws.Cell(1, 3).Value = "Fecha Actualización";
        ws.Cell(1, 4).Value = "Detalle de Cambio";
        ws.Cell(1, 5).Value = "Salario";
        ws.Cell(1, 6).Value = "Número de Empleado";
        ws.Cell(1, 7).Value = "Nombre Completo";
        ...
        foreach (dynamic x in data)
        {
            ...
            ws.Cell(row, 2).Value = x.Usuario;
            ws.Cell(row, 3).Value = x.FechaActualizacion;
            ws.Cell(row, 4).Value = x.DetalleCambio;
            ws.Cell(row, 5).Value = x.Salario;
            ws.Cell(row, 6).Value = x.EmpNo;
            ws.Cell(row, 7).Value = x.Nombre;
            ws.Cell(row, 8).Value = x.Departamento;
            row++;
        }
        ...
        var pdf = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Content().Table(table =>
                {
                    ...
                    foreach (var x in data)
                    {
                        table.Cell().Text(x.DeptNo);
                        table.Cell().Text(x.Employee.FirstName + " " + x.Employee.LastName);
                        table.Cell().Text(x.FromDate.ToString("yyyy-MM-dd"));
                        table.Cell().Text(x.ToDate.HasValue ? x.ToDate.Value.ToString("yyyy-MM-dd") : "");
                    }
                });
            });
        }).GeneratePdf();
